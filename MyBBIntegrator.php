...
class MyBBIntegrator
{
    ...
    {
        echo '<div style="width:92%; margin:4px auto; border:1px #DDD solid; background:#F1F1F1; padding:5px; color:#C00; font-weight:bold;">An error occured during script run.<br />' .
            $message . '</div>';
        die;
    }
    ...
    function checkForumPassword($forum_id, $password = '', $pid = 0)
    {
        ...
        if ($forum_password) {
            if (empty($password)) {
                if (!$this->mybb->cookies['forumpass'][$forum_id] || ($this->mybb->cookies['forumpass'][$forum_id] &&
                        md5($this->mybb->user['uid'] . $forum_password) != $this->mybb->cookies['forumpass'][$forum_id])
                ) {
                    return false;
                } else {
                    return true;
                ...
                if ($forum_password == $password) {
                    $this->setCookie('forumpass[' . $forum_id . ']', md5($this->mybb->user['uid'] .
                            $password), null, true);
                    return true;
                } else {
                    return false;
    ...
    function closeThread($thread_id, $forum_id = 0)
    {
        ...
        $this->MyBBIntegratorClassObject('moderation', 'Moderation', MYBB_ROOT .
     ...
     */
    function createCategory($data, $permissions = array(), $default_permissions =
    array())
    {
        ...
        if (!empty($permissions)) {
            if ((!isset($permissions['canview']) || empty($permissions['canview'])) || (!
        ...
        $this->db->update_query("forums", array("parentlist" => $data['parentlist']),
            'fid=\'' . $data['fid'] . '\'');
    ...
    function createPoll($thread_id, $data)
    {
        ...
        if (($thread['uid'] != $this->mybb->user['uid'] && !is_moderator($fid)) || ($forumpermissions['canview'] ==
                0 || $forumpermissions['canpostthreads'] == 0 || $forumpermissions['canpostpolls'] ==
                0)
        ) {
            return false;
        }
        ...
        if ($this->mybb->settings['maxpolloptions'] && $polloptions > $this->mybb->
                settings['maxpolloptions']
        ) {
        ...
        for ($i = 0; $i < $polloptions; ++$i) {
            ...
            if (my_strlen($options[$i]) > $this->mybb->settings['polloptionlimit'] && $this->
    ...
    function deletePrivateMessage($pm_id)
    {
        ...
        if (is_array($pm_id)) {
            $this->db->update_query('privatemessages', $data, 'pmid IN (' . implode(',',
                    array_map('intval', $pm_id)) . ')');
        } else {
    ...
    function deletePrivateMessagesOfUser($user_id)
    {
        ...
        if (is_array($user_id)) {
            $this->db->update_query('privatemessages', $data, 'uid IN (' . implode(',',
    ...
    function generateCaptcha()
    {
        ...
        return array_merge($imagearray, array('captcha' => '<img src="' . $this->mybb->
                settings['bburl'] . '/captcha.php?imagehash=' . $imagehash . '" />'));
    }
     ...
     * @return array
     */
    function getBusyThreadsWithinTimespan($timespan = 86400, $post_sort_order =
    'DESC', $postamount_sort_order = 'DESC')
    {
        $threads = array();
    ...
    function getForum($forum_id, $active_override = 0)
    {
        ...
        $forumpermissions = forum_permissions($forum['fid']);
        if ($forumpermissions['canview'] != 1 || $forumpermissions['canviewthreads'] !=
            1
        ) {
            // error_no_permission();
            return false;
    ...
    function getForumStaff()
    {
        ...
        $query = $this->db->simple_select("usergroups", "gid, title, usertitle",
            "showforumteam=1", array('order_by' => 'disporder'));
        ...
        $query = $this->db->simple_select("users",
            "uid, username, displaygroup, usergroup, ignorelist, hideemail, receivepms",
    ...
    function getLatestActiveThreads($forum_id = 0, $limit = 7, $exclude_invisible = true)
    {
        ...
        $condition = ($forum_id != 0) ? ' `visible` = ' . $fetch_invisible_threads .
            ' AND `fid` = ' . intval($forum_id) : '';
     ...
     * @return array
     */
    function getLatestThreads($forum_id = 0, $fields = '*', $limit = 7, $exclude_invisible = true,
                              $join_forums = true, $join_first_post = true)
    {
        if ($forum_id != 0) {
            ...
            if (is_array($forum_id)) {
                foreach ($forum_id as $single_forum_id) {
                    ...
                    if ($forum_permissions['canview'] != 1 || $forum_permissions['canviewthreads'] !=
                        1
                    ) {
                        // error_no_permission();
                        return false;
        ...
        $forum_join = ($join_forums == true) ? 'INNER JOIN ' . TABLE_PREFIX .
            'forums f ON f.`fid` = t.`fid`' : '';

        // Do we want to get the first post from the thread?
        $first_post_join = ($join_first_post == true) ? 'INNER JOIN ' . TABLE_PREFIX .
    ...
    /** Funcion custom de Minekkit (28/01/2014)
     * Devuelve la cantidad de usuarios del foro
     *
     * @return integer
     */
    function getMembersCount()
    {
        ...
        while ($cant = $this->db->fetch_array($query)) {
            $c[] = $cant;
        }
    ...
    function getMembers($data = array())
    {
        ...
        $data['perpage'] = (!isset($data['perpage'])) ? (int)$this->mybb->settings['membersperpage'] : (int)
        $data['perpage'];
     ...
     * @return array|boolean: If unsuccessful, it returns false - Otherwise the Database row
     */
    function getPost($post_id, $parsed = false, $override_forum_parse_options =
    array())
    {
        if ($post_id == 0) {
        ...
        if (count($override_forum_parse_options) == 0) {
            ...
            $parser_options = array(
                'allow_html' => (isset($override_forum_parse_options['allow_html']) && $override_forum_parse_options['allow_html'] ==
                        1) ? 1 : 0,
                'allow_mycode' => (isset($override_forum_parse_options['allow_mycode']) && $override_forum_parse_options['allow_mycode'] ==
                        1) ? 1 : 0,
                'allow_smilies' => (isset($override_forum_parse_options['allow_smilies']) && $override_forum_parse_options['allow_smilies'] ==
                        1) ? 1 : 0,
                'allow_imgcode' => (isset($override_forum_parse_options['allow_imgcode']) && $override_forum_parse_options['allow_imgcode'] ==
                        1) ? 1 : 0,
                'filter_badwords' => (isset($override_forum_parse_options['filter_badwords']) &&
                        $override_forum_parse_options['filter_badwords'] == 1) ? 1 : 0,
            );
        }
    ...
    function getPostsOfThread($thread_id, $fields = '*', $options = array())
    {
        ...
        $query_thread = $this->db->query('SELECT `fid` FROM ' . TABLE_PREFIX .
            'threads WHERE `tid` = ' . intval($thread_id) . ' LIMIT 1');
        ...
        $query = $this->db->simple_select('posts', $fields, '`tid` = ' . intval($thread_id),
            $options);
     ...
     * @return array
     */
    function getPrivateMessagesOfUser($user_id, $params = array('orderby' =>
        'pm.dateline', 'sort' => 'DESC'), $translate_folders = true)
    {
        /**
         * This is what we will be returning
     ...
     * @return array
     */
    function getThreads($forum_id, $fields = '*', $where = '', $query_params = array
                                 ('order_by' => 't.`subject`', 'order_dir' => 'ASC'), $exclude_invisible = true,
                        $join_forums = false, $join_first_post = false)
    {
        // Do we have permission?
        if (!is_array($forum_id)) {
            $forumpermissions = forum_permissions($forum_id);
        ...
        // Is a Limit defined?
        $limit = (isset($query_params['limit_start']) && isset($query_params['limit'])) ?
            'LIMIT ' . intval($query_params['limit_start']) . ', ' . intval($query_params['limit']) :
            '';
    ...
    function claimCoin($username = '')
    {
        ...
        $query = $this->db->query('
			...
			FROM ' . TABLE_PREFIX . 'users u 
			WHERE u.`username` = "' . $username . '" 
            AND ((u.money >=35 AND NOT u.usergroup = 23) OR (u.money>=50 AND u.usergroup=23))
    ...
    function getWhoIsOnline($colored_usernames = true)
    {
        ...
        if ($this->mybb->settings['showwol'] != 0 && $this->mybb->usergroup['canviewonline'] !=
            0
        ) {
            ...
            while ($user = $this->db->fetch_array($query)) {
                ...
                if ($user['uid'] > 0) {
                    ...
                    if ($doneusers[$user['uid']] < $user['time'] || !$doneusers[$user['uid']]) {
                        ...
                        if ($user['invisible'] != 1 || $this->mybb->usergroup['canviewwolinvis'] == 1 ||
                            $user['uid'] == $this->mybb->user['uid']
                        ) {
    ...
    function getWhoPosted($thread_id, $sort_by_posts = true)
    {
        ...
        while ($poster = $this->db->fetch_array($query)) {
            ...
            $arr[$i]['profile_link'] = build_profile_link($poster_name, $poster['uid'],
                '_blank', $onclick);
    ...
    function isModerator($forum_id = 0, $action = '', $user_id = 0)
    {
        ...
        return is_moderator($forum_id, $action, ($user_id == 0) ? $this->mybb->user['uid'] :
            $user_id);
    }
    ...
    function login($username, $password, $captcha_hash = '', $captcha_string = '')
    {
         ...
         */
        $query = $this->db->simple_select("users", "loginattempts", "LOWER(username)='" .
            $this->db->escape_string(my_strtolower($username)) . "'", array('limit' => 1));
        ...
        if (!$user['uid']) {
            my_setcookie('loginattempts', $logins + 1);
            $this->db->write_query("UPDATE " . TABLE_PREFIX .
                "users SET `loginattempts` = `loginattempts` + 1 WHERE LOWER(`username`) = '" .
                $this->db->escape_string(my_strtolower($username)) . "'");
            $bad_login = true;
        }
        ...
        if ($loginattempts > 3 || intval($this->$mybb->cookies['loginattempts']) > 3) {
            ...
            if (!empty($captcha_hash) && !empty($captcha_string)) {
                ...
                if ($this->mybb->settings['captchaimage'] == 1 && function_exists("imagepng") &&
                    !$this->mybb->user['uid']
                ) {
                    return $this->generateCaptcha();
                }
        ...
        $this->db->delete_query("sessions", "ip='" . $this->db->escape_string($this->
                mybb->session->ipaddress) . "' AND sid != '" . $this->mybb->session->sid . "'");

        // Create a new session and save it in the database
        $newsession = array("uid" => $user['uid'],);
        $this->db->update_query("sessions", $newsession, "sid='" . $this->mybb->session->
                sid . "'");
    ...
    function logout()
    {
        ...
        if ($this->mybb->input['sid'] && $this->mybb->input['sid'] != $this->mybb->
                session->sid
        ) {
            return false;
        } // Otherwise, check logoutkey
        else
            if (!$this->mybb->input['sid'] && $this->mybb->input['logoutkey'] != $this->
                    mybb->user['logoutkey']
            ) {
                return false;
            }
    ...
    function parseString($message, $options = array())
    {
        ...
        if ($options['allow_html'] != 1) {
            ...
            while (preg_match("#<script(.*)>(.*)</script(.*)>#is", $message)) {
                $message = preg_replace("#<script(.*)>(.*)</script(.*)>#is", "&lt;script$1&gt;$2&lt;/script$3&gt;",
                    $message);
            }
        ...
        if ($options['me_username']) {
            $message = preg_replace('#(>|^|\r|\n)/me ([^\r\n<]*)#i', "\\1<span style=\"color: red;\">* {$options['me_username']} \\2</span>",
                $message);
            $message = preg_replace('#(>|^|\r|\n)/slap ([^\r\n<]*)#i', "\\1<span style=\"color: red;\">* {$options['me_username']} {$this->lang->slaps} \\2 {$this->lang->with_trout}</span>",
                $message);
        }
    ...
    function register($info = array())
    {
        ...
        if ($this->mybb->settings['regtype'] == "verify" || $this->mybb->settings['regtype'] ==
            "admin" || $info['coppa'] == 1
        ) {
            $usergroup = 5;
        } else {
        ...
        if ($this->mybb->cookies['coppauser']) {
            $this->lang->redirect_registered_coppa_activate = $this->lang->sprintf($this->
                lang->redirect_registered_coppa_activate, $this->mybb->settings['bbname'], $user_info['username']);
         ...
         */ else
            if ($this->mybb->settings['regtype'] == "verify") {
                ...
                $emailsubject = $this->lang->sprintf($this->lang->emailsubject_activateaccount,
                    $this->mybb->settings['bbname']);
                $emailmessage = $this->lang->sprintf($this->lang->email_activateaccount, $user_info['username'],
                    $this->mybb->settings['bbname'], $this->mybb->settings['bburl'], $user_info['uid'],
                    $activationcode);
                my_mail($user_info['email'], $emailsubject, $emailmessage);

                // Build the message to return
                $this->lang->redirect_registered_activation = $this->lang->sprintf($this->lang->
                    redirect_registered_activation, $this->mybb->settings['bbname'], $user_info['username']);
             ...
             */ else
                if ($this->mybb->settings['regtype'] == "randompass") {
                    // Generate and send the email
                    $emailsubject = $this->lang->sprintf($this->lang->emailsubject_randompassword, $this->
                        mybb->settings['bbname']);
                    $emailmessage = $this->lang->sprintf($this->lang->email_randompassword, $user['username'],
                        $this->mybb->settings['bbname'], $user_info['username'], $user_info['password']);
                    my_mail($user_info['email'], $emailsubject, $emailmessage);
                 ...
                 */ else
                    if ($this->mybb->settings['regtype'] == "admin") {
                        // Build the message to return
                        $this->lang->redirect_registered_admin_activate = $this->lang->sprintf($this->
                            lang->redirect_registered_admin_activate, $this->mybb->settings['bbname'], $user_info['username']);
                     ...
                     */ else {
                        // Build the message to return
                        $this->lang->redirect_registered = $this->lang->sprintf($this->lang->
                            redirect_registered, $this->mybb->settings['bbname'], $user_info['username']);
    ...
    function removePost($post_id)
    {
        ...
        $query = $this->db->simple_select("posts", "pid", "tid='{$tid}'", array(
            "limit" => 1,
            "order_by" => "dateline",
            "order_dir" => "asc"));
        ...
        if ($firstpost) {
            ...
            if ($forumpermissions['candeleteposts'] == 1 || is_moderator($fid,
                    "candeleteposts")
            ) {
                ...
                $query = $this->db->simple_select("posts", "pid", "tid='{$tid}' AND dateline <= '{$post['dateline']}'",
                    array(
                        "limit" => 1,
                        "order_by" => "dateline",
                        "order_dir" => "desc"));
    ...
    function removeUser($user)
    {
        ...
        if (!is_numeric($user)) {
            $query = $this->db->simple_select('users', 'uid', 'username=\'' . $this->
                    dbEscape($user) . '\'');
    ...
    function sendPrivateMessage($data = array())
    {
        ...
        $query = $this->db->query("
			...
			WHERE u.username='" . $this->db->escape_string($data['to_uername']) .
            "' AND pm.dateline > {$time_cutoff} AND pm.fromid='{" . $data['fromid'] .
            "}' AND pm.subject='" . $this->db->escape_string($data['subject']) .
            "' AND pm.message='" . $this->db->escape_string($data['message']) .
            "' AND pm.folder!='3'
		");
    ...
    function updatePasswordOfUser($user_id, $password, $inline_error = true)
    {
        ...
        $data = array('uid' => intval($user_id), 'password' => $password);
    ...
    function updateMoney($user_id, $money, $inline_error = true)
    {
        ...
        $data = array('uid' => intval($user_id), 'money' => $money);
    ...
    function vote($poll_id, $user_id = 0, $option = null)
    {
        ...
        if ($poll['closed'] == 1 || $thread['closed'] == 1 || ($expiretime < TIME_NOW &&
                $poll['timeout'])
        ) {
        ...
        if ($user_id > 0) {
            $query = $this->db->simple_select("pollvotes", "*", "uid='" . $user_id .
                "' AND pid='" . $poll['pid'] . "'");
        ...
        if ($poll['multiple'] == 1) {
            foreach ($option as $voteoption => $vote) {
                if ($vote == 1 && isset($votesarray[$voteoption - 1])) {
                    ...
                    $votesql .= "('" . $poll['pid'] . "','" . $user_id . "','" . $this->db->
                            escape_string($voteoption) . "', " . TIME_NOW . ")";
